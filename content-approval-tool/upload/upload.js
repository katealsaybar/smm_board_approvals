...
// Same allowlist as ../view/app.js — RLS requires one of these 4 signed in.
...
const revisionDestField = document.getElementById('revisionDestField');
const revisionDestSelect = document.getElementById('revisionDestSelect');
const smmNotesInput = document.getElementById('smmNotesInput');
...
async function loadBatches(){
  ...
  batchSelect.innerHTML =
    batchesCache.map(b => `<option value="${b.id}">${escapeHtml(b.name)}</option>`).join('') +
    `<option value="__new__">+ Create new batch</option>`;
  renderRevisionDestOptions();
}
...
// Defaults to whichever batch the target item is already in — Jules only needs to
// change this when the fix should be picked up in a newer review round instead.
function renderRevisionDestOptions(){
  const target = currentRevisionTarget();
  revisionDestSelect.innerHTML = batchesCache.map(b =>
    `<option value="${b.id}" ${target && b.id === target.batchId ? 'selected' : ''}>${escapeHtml(b.name)}</option>`
  ).join('');
}
...
modeSelect.addEventListener('change', ()=>{
  ...
  revisionDestField.style.display = isRevision ? 'block' : 'none';
  toggleUploadMode();
});
...
submitBtn.addEventListener('click', async ()=>{
  ...
  const smmNotes = smmNotesInput.value.trim();

  let category, format, batchId, batchName, isNewBatch = false, newName = '', revisionTarget = null, destBatchId = null;

  if (isRevision){
    revisionTarget = currentRevisionTarget();
    if (!revisionTarget){ setStatus('error', 'Pick which content this fix belongs to.'); return; }
    destBatchId = revisionDestSelect.value;
    if (!destBatchId){ setStatus('error', 'Pick which batch this should land in.'); return; }
    category = revisionTarget.category;
    format = revisionTarget.format;
    const destBatch = batchesCache.find(b => b.id === destBatchId);
    batchName = destBatch ? destBatch.name : revisionTarget.batchName;
  } else {
  ...
  try{
    ...
    if (isRevision){
      const isMove = destBatchId !== revisionTarget.batchId;
      const { error: revErr } = await sb.from('revisions').insert({
        ...
        smm_notes: smmNotes || null,
        moved_from_batch_id: isMove ? revisionTarget.batchId : null,
        moved_to_batch_id: isMove ? destBatchId : null
      });
      if (revErr) throw new Error(revErr.message);

      if (isMove){
        const { error: moveErr } = await sb.from('content_items').update({ batch_id: destBatchId }).eq('id', revisionTarget.id);
        if (moveErr) throw new Error(moveErr.message);
      }

      setStatus('success', isMove
        ? `New version added and moved into "${escapeHtml(batchName)}" — reviewers will see it there as v${revisionTarget.latestRevisionNumber + 1}.`
        : `New version added — reviewers will see it as v${revisionTarget.latestRevisionNumber + 1} on the same card.`);
    } else {
